...
const Industries = () => {
  ...
  return (
    <section id="industries" className="py-10 bg-transparent scroll-mt-20">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-serif text-slate-900 font-light">Industries <span className="italic text-aurora-teal">We Serve</span></h2>
        </div>
          ...
          {industryList.map((industry, index) => (
            <div key={index} className="group relative h-[450px] rounded-[3rem] overflow-hidden shadow-2xl transition-all duration-700 hover:-translate-y-4">
              ...
              <div className="absolute inset-0 z-10 p-10 flex flex-col justify-end">
                ...
                <h3 className="text-2xl font-bold text-white leading-tight tracking-tight group-hover:text-aurora-teal transition-colors duration-300">{industry.name}</h3>
                <div className="mt-4 overflow-hidden h-0 group-hover:h-6 transition-all duration-500">
                  <span className="text-[10px] uppercase font-black tracking-[0.4em] text-white/80">Expert Solutions</span>
                </div>
              </div>
            </div>
          ))}
